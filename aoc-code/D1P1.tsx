...
const data = ['nkzjrdqrmpztpqninetwofour1znnkd',
    ...
    //Day 1 - Puzzle 1
    const output1 = data.map(function(x) {
        const justNums = x.replace(/[^0-9]/g, '')
        const countNums = justNums.replace(/[^0-9]/g, '').length
        const first = x.replace(/[^0-9]/g, '').charAt(0)
        const second = x.replaceAll(/[^0-9]/g, '').charAt(countNums - 1);
        if (countNums > 1){
    ...
    let sum1 = output1.reduce((a, b) => a + b, 0);

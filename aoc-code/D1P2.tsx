...
const data = ['nkzjrdqrmpztpqninetwofour1znnkd',
    ...
    //Day 1 - Puzzle 2
    const output2 = data.map(function(x) {
        const stringy = x.replaceAll("one","one1one").replaceAll("two","two2two").replaceAll("three","three3three").replaceAll("four","four4four").replaceAll("five","five5five").replaceAll("six","six6six").replaceAll("seven","seven7seven").replaceAll("eight","eight8eight").replaceAll("nine","nine9nine")
        const justNums = stringy.replace(/[^0-9]/g, '')
        const countNums = justNums.length
        const first = justNums.charAt(0)
        const second = justNums.charAt(countNums - 1);
        if (countNums > 1){
            return parseInt(first + second)
        } else if (countNums == 1){
            return parseInt(justNums + justNums)
        }
    });
    let sum2 = output2.reduce((a, b) => a + b, 0);
